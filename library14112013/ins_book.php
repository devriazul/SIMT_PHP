...
{ 
  if($_SESSION['userid']){ 
	...
	$chkbook=$myDb->select("SELECT id,courseid,deptid FROM tbl_bookentry WHERE courseid='$courseid' AND deptid='$deptid'");
	...
	if(empty($bookf['courseid']) && empty($bookf['deptid'])){
		...
		if($r=$myDb->insert_sql($query)){
			...
			$pquery="INSERT INTO tbl_product(pname,prtype,courseid,bookid,opby)
					 VALUES('$coursef[coursename]','$deptf[name]','$coursef[id]','$maxbookf[mid]','$_SESSION[userid]')";
