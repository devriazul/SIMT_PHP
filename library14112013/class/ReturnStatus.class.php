...
class ReturnStatus extends dbClass{
  ...
  public function CurrentDateReturn($qry,$act=''){
	...
	echo "<table class='class-tbl-style' border='0' cellpadding='0' cellspacing='0'>";
   ...
   $i=0;
	while($this->r=parent::get_row($this->qry,'MYSQL_ASSOC')){
	  if($i==0){
		  ...
		  foreach($this->r as $key=>$value){
			 echo "<td class='headline'>".strtoupper($key)."</td>";	  
		  
		  }
		  if(!empty($act)){
		     echo "<td colspan=2 class='headline'>Action</td>";
		  }
		 "</tr>";
	  }
	  $i++;
	 ...
	 foreach($this->r as $key=>$value){
	    ...
	    if($key!="stdname"){
		  ...
		  //echo "<td class='record-headline'><a href=issRtnRpt.php?".$this->getFieldName()."=".urlencode($this->getFieldValue()).">".$value."</a></td>";
	    		  echo "<td class='record-headline'><a href='#' name='prt' id='".$this->getFieldName()."' class='".$v."' >".$value."</a></td>";

		}else{
		  echo "<td class='record-headline'>".$value."</td>";
	...
	if(!empty($act)){
		echo "<td class='record-headline'>Edit</td>";
		echo "<td class='record-headline'>Delete</td>";
	}
	echo "</tr>";
	}
	echo "</table>";
