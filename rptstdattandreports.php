...
*/
//echo $_POST['deptid'].$_POST['courseid'].$_POST['semester'].$_POST['session']; exit;
if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['session'])!="" && ($_POST['courseid']!=""))
{ //echo "1"; exit;
 	$crs="Select COUNT(sar.id) as tn From tbl_stdattandence sar inner join tbl_stdinfo s on sar.stdid=s.stdid Where sar.deptid='$_POST[deptid]' and sar.courseid='$_POST[courseid]' and sar.semesterid='$_POST[semester]' and sar.session='$_POST[session]' and sar.storedstatus<>'D'";
 	$crq=$myDb->select($crs); 
  	$crsr=$myDb->get_row($crq,'MYSQL_ASSOC');
	$pdf->SetFont('Arial',  'I', 10);
	$pdf->Cell(-175, -470, 'Total No of students: '.$crsr['tn'],0,'C',0);
...
*/
	...
	$pdf->SetFont('Arial', '');
	$pdf->Cell(0, -470, "Date: ".date('F j, Y'), 0,0, 'R');

// set the Report Header & Date
$pdf->SetFont('Arial', 'B', 16);
$pdf->SetFillColor(235,234,230);
$pdf->Cell(-160,-540,"SAIC INSTITUTE OF MANAGEMENT & TECHNOLOGY",0,0,'C');
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(150,-530,"House # 1, Road # 2, Block # B, Section # 6, Mirpur, Dhaka - 1216",0,0,'C');
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(-150,-520,"Phone: +88 02 8033034, +88 01936005816, +88 01936005817",0,0,'C');
...
if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['session'])!="" && ($_POST['courseid']!=""))
{
 	...
 	$src="SELECT coursename, coursecode From tbl_courses WHERE id='$_POST[courseid]' and storedstatus<>'D'";
 	$src=$myDb->select($src); 
  	$srsc=$myDb->get_row($src,'MYSQL_ASSOC');

	$pdf->Cell(150,-500,"Student Attendance Report",0,0,'C');
	$pdf->Cell(-150,-490,"(Department: ".$drsr['name']."/ Semester: ".$srsr['name']."/ Course: ".$srsc['coursename'].")",0,0,'C');

}

/*
if(($_POST['sexstatus']!="") && ($_POST['semester']=="") && ($_POST['stdstatus'])=="" && ($_POST['section']=="") && ($_POST['session']=="") && ($_POST['stdresultstatus']==""))
{
...
$pdf->SetX(5);
		...
		$pdf->Cell(25, 6, 'Student ID', 1, 0, 'L', 1);
		$pdf->Cell(55, 6, 'Student Name', 1, 0, 'L', 1);
		$pdf->Cell(25, 6, 'Section', 1, 0, 'C', 1);
		$pdf->Cell(35, 6, 'Total Working Days', 1, 0, 'C', 1);
		$pdf->Cell(35, 6, 'Total Attend Days', 1, 0, 'C', 1);
		$pdf->Cell(25, 6, 'Attendance(%)', 1, 0, 'C', 1);
		//$pdf->Cell(25, 6, 'Semester', 1, 0, 'L', 1);
		//$pdf->Cell(15, 6, 'Board Roll', 1, 0, 'C', 1);
		//$pdf->Cell(15, 6, 'Board Reg.', 1, 0, 'C', 1);
		//$pdf->Cell(19, 6, 'Hostel', 1, 0, 'L', 1);
...
if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['session'])!="" && ($_POST['courseid']!=""))
{
	$result=mysql_query("Select s.stdid as StudentID, s.stdname as Name, s.section as Section, sar.totalworkingdays as TotalWorkingDays, sar.attndays as PresentDays, sar.attendancepercent as 'Attendance(%)' From tbl_stdattandence sar inner join tbl_stdinfo s on sar.stdid=s.stdid Where sar.deptid='$_POST[deptid]' and sar.courseid='$_POST[courseid]' and sar.semesterid='$_POST[semester]' and sar.session='$_POST[session]' and sar.storedstatus<>'D' order by sar.section");
}
/*
else if(($_POST['deptid']!="") && ($_POST['semester']=="") && ($_POST['stdstatus']=="") && ($_POST['section']=="") && ($_POST['sexstatus']=="") && ($_POST['session']!="") && ($_POST['stdresultstatus']==""))
{
...
while($row = mysql_fetch_array($result))
{
    ...
    $pdf->Cell(25, 6, $stdid, 1, 0, 'L', 1);
    $pdf->Cell(55, 6, $stdname, 1, 0, 'L', 1);
    $pdf->Cell(25, 6, $section, 1, 0, 'C', 1);
    $pdf->Cell(35, 6, $totalworkingdays, 1, 0, 'C', 1);
    $pdf->Cell(35, 6, $attndays, 1, 0, 'C', 1);
    $pdf->Cell(25, 6, $attendancepercent, 1, 0, 'C', 1);
    //$pdf->Cell(25, 6, $semester, 1, 0, 'L', 1);
