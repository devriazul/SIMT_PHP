...
//Position at 1.5 cm from bottom
    ...
    $pdf->Cell(280,10,'� Saic Institute of Management & Technology. Powered By: <a href="https://riaz.fastitbd.com">(Web Developer) </a><a href="https://www.saicgroupbd.com">Saic Group</a>',0,0,'C');
...
if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['section']!="") && ($_POST['stdstatus']!="") && ($_POST['sexstatus']=="") && ($_POST['session']!="") && ($_POST['stdresultstatus']==""))
{//echo "4"; exit;
 	$crs="SELECT COUNT(s.id) as tn FROM `tbl_stdinfo` s inner join tbl_department d on s.deptname=d.id inner join tbl_batch b on s.batch=b.id inner join tbl_semester sm on s.stdcursemester=sm.id inner join tbl_hostel h on s.hostelid=h.id WHERE s.deptname='$_POST[deptid]' and s.stdcursemester='$_POST[semester]' and s.section='$_POST[section]' and s.stdstatus='$_POST[stdstatus]' and s.session='$_POST[session]' and s.storedstatus<>'D' ";
 	$crq=$myDb->select($crs); 
  	$crsr=$myDb->get_row($crq,'MYSQL_ASSOC');
	$pdf->SetFont('Arial',  'I', 10);
	$pdf->Cell(-245, -300, 'Total No of students: '.$crsr['tn'],0,'C',0);
...
{//echo "4"; exit;
	$s=$_POST['semester']+1;
 	...
 	$crs="SELECT COUNT(s.id) as tn FROM `tbl_stdinfo` s inner join tbl_department d on s.deptname=d.id inner join tbl_batch b on s.batch=b.id inner join tbl_semester sm on s.stdcursemester=sm.id inner join tbl_hostel h on s.hostelid=h.id WHERE s.deptname='$_POST[deptid]' and s.stdcursemester='$s' and s.section='$_POST[section]' and s.resultstatus='$_POST[stdresultstatus]' and s.session='$_POST[session]' and s.storedstatus<>'D' ";
 	$crq=$myDb->select($crs); 
  	$crsr=$myDb->get_row($crq,'MYSQL_ASSOC');
	$pdf->SetFont('Arial',  'I', 10);
	$pdf->Cell(-245, -300, 'Total No of students: '.$crsr['tn'],0,'C',0);
...
if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['section']!="") && ($_POST['stdstatus']!="") && ($_POST['sexstatus']=="") && ($_POST['session']!="") && ($_POST['stdresultstatus']==""))
{
	...
	if($_POST['stdstatus']=="D")
	{
		$t="Drop Out";
	}
	else if($_POST['stdstatus']=="R")
	{
		$t="Regular";
	}
	else if($_POST['stdstatus']=="IR")
	{
		$t="Irregular";
	}
	
	else if($_POST['stdstatus']=="C")
	{
		$t="Cancel";
	}
	...
	$pdf->Cell(-100,-320,"Student Result Information Report",0,0,'C');
	$pdf->Cell(100,-310," Department: ".$drsr['name']."/ Semester: ".$srsr['name']."/ Session: ".$_POST['session']."/ Section: ".$_POST['section']."/ Type: ".$t,0,0,'C');
...
if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['section']!="") && ($_POST['stdstatus']=="") && ($_POST['sexstatus']=="") && ($_POST['session']!="") && ($_POST['stdresultstatus']!=""))
{
	...
	$pdf->Cell(-100,-320,"Student Result Information Report",0,0,'C');
	$pdf->Cell(100,-310," Department: ".$drsr['name']."/ Semester: ".$srsr['name']."/ Session: ".$_POST['session']."/ Section: ".$_POST['section']."/ Result: ".$_POST['stdresultstatus'],0,0,'C');
...
{
	$s=$_POST['semester']+1;
	$result=mysql_query("SELECT s.*, d.name as department, b.batchname as batchname, sm.name as semester, h.name as hostel FROM `tbl_stdinfo` s inner join tbl_department d on s.deptname=d.id inner join tbl_batch b on s.batch=b.id inner join tbl_semester sm on s.stdcursemester=sm.id inner join tbl_hostel h on s.hostelid=h.id WHERE s.deptname='$_POST[deptid]' and s.stdcursemester=$s and s.section='$_POST[section]' and s.resultstatus='$_POST[stdresultstatus]' and s.session='$_POST[session]' and s.storedstatus<>'D'  Order By d.id");
}
else if(($_POST['deptid']!="") && ($_POST['semester']!="") && ($_POST['section']!="") && ($_POST['stdstatus']!="") && ($_POST['sexstatus']=="") && ($_POST['session']!="") && ($_POST['stdresultstatus']==""))
{
	//$s=$_POST['semester']+1;
	$result=mysql_query("SELECT s.*, d.name as department, b.batchname as batchname, sm.name as semester, h.name as hostel FROM `tbl_stdinfo` s inner join tbl_department d on s.deptname=d.id inner join tbl_batch b on s.batch=b.id inner join tbl_semester sm on s.stdcursemester=sm.id inner join tbl_hostel h on s.hostelid=h.id WHERE s.deptname='$_POST[deptid]' and s.stdcursemester='$_POST[semester]' and s.section='$_POST[section]' and s.stdstatus='$_POST[stdstatus]' and s.session='$_POST[session]' and s.storedstatus<>'D'  Order By d.id");
}
/*
else
{
...
while($row = mysql_fetch_array($result))
{
    ...
    if ($i == $max)
    {
		...
		$y_axis = 30;
        $pdf->AddPage('L');
      	$pdf->Cell(280,380,'� Saic Institute of Management & Technology. Powered By: <a href="https://riaz.fastitbd.com">(Web Developer) </a><a href="https://www.saicgroupbd.com">Saic Group</a>.',0,0,'C');
