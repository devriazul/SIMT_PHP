...
{ 
  ...
  if(($car['ins']=="y")||($_SESSION['userid']=="administrator")){
	...
	$f=mysql_query("select*from tbl_faculty where id='$id'") or die(mysql_error());
	$ff=mysql_fetch_array($f);
	/*--------thr shud b a chicking with leave table, salary table n attendence table -----------
	...
	if(($htrow['psid']==$htrow['payscaleid'])&&($htrow['psid']!="")&&($htrow['payscaleid']!="")){
	   ...
	   mysql_query("delete from tbl_login where userid='$ff[facultyid]'") or die(mysql_error());
		//---------Delete data from `tbl_attendance`---------------
	   mysql_query("delete from tbl_attendance where efid='$ff[facultyid]'") or die(mysql_error());
	   ...
	   //mysql_query("delete from tbl_employeesalary where empid='$ff[sid]'") or die(mysql_error());
		//---------Delete data from `tbl_faculty`---------------
	   mysql_query("delete from tbl_faculty where facultyid='$ff[facultyid]'") or die(mysql_error());

	   //$qup="UPDATE tbl_faculty SET `storedstatus`='D' WHERE `id`='$id'";
