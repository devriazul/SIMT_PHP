...
class Utility extends dbClass{
   ...
   public function currentIssueid(){
      //$R="REQ-10000022222"; $rl=strlen($R);  $sp=strpos($R,"-"); echo $sub=substr($R,4,$rl);  ?
		  $this->q=parent::select("SELECT isuid FROM tbl_issue where id=(select max(id) from tbl_issue)");
		  $this->rs=parent::get_row($this->q,'MYSQL_ASSOC');

		if(empty($this->rs["isuid"])){
			 $this->r="ISU-10001";
			 $this->rl=strlen($this->r);
			 $this->sp=strpos($this->r,"-");
			 $this->sub=substr($this->r,4,$this->rl);
			 ...
			 $this->r=$this->rs["isuid"];
			 $this->rl=strlen($this->r);
			 $this->sp=strpos($this->r,"-");
			 $this->sub=substr($this->r,4,$this->rl);
			 $this->isuid="ISU-".intval(($this->sub)+1);
			 return $this->isuid;
		} 
   }
   public function formatText($textBoxValue,$flaging){
		$prtype=mysql_real_escape_string($textBoxValue);
		  switch($flaging){
			case "1":
