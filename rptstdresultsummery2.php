...
<?php //All page heading (First,Last page not here)
		...
		echo "<div align='right' style='float:right; font-size:12px;width:500px;'>Examination Year: {$heldmonths} {$year}</div>";
...
<?php 	$crs=$myDb->select("SELECT distinct d.id,d.name,c.coursename,c.coursecode,s.name,ss.session,ss.year
	   ...
	   echo "<table cellpadding='0' cellspacing='0' style=\"border-bottom:1px solid #666;border-left:1px solid #666;\">";
	   echo "<tr>";
	  ...
	  while($stqf=$myDb->get_row($stq,'MYSQL_ASSOC')){	  
		  ...
		  echo "<table class='stdhead' cellpadding='0' cellspacing='0'>";
			echo "<tr>";
			echo "<td >".$stqf['stdname']."<br/>".$stqf['boardrollno']."<br/>".$stqf['boardregno']."<br/>".$stqf['session']."</td>";
			echo "</tr>";	  
			echo "</table>";
			echo "</td>";
...
/*				
				...
				if(($incsf['cont_assess_t']>0&&$incsf['f_exam_t']>0&&$incsf['cont_assess_p']>0&&$incsf['f_exam_p']>0)){	
					...
					if($totalTFC>=40&&$totalPFC>=40){
						echo "<td align='center' class='head'>".$total."<br/><hr><br/>".$cgpa."<br/><hr><span style='color: #009900'>".$grade."</span></td>";
					}else{
						$grade="F";
	...
	?>
			...
			<td align="center" class="head" width="50%"><?php echo $status; ?><br/> 
