...
<script language="javascript" src="show_courseattnd.js"></script>
<script language="javascript">
 $(document).ready(function(){
   $('#sbt').click(function(){
	var courseid=$('#courseid').val();
    var arr=$('#MyForm').serializeArray();	 
	$.post('submitstudentattndmarks_search.php?courseid='+courseid,arr,function(result){
...
<div class="transbox"><div id="bothead" style=" position:absolute; background-color:#666666;   padding:10px; color:#FFFFFF; left:400px;float:right;width:700px; height:auto; display:none; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px;">
  ...
  ---------------------------------------------<br/><br/>
   ...
   <label>* Select a Exam Type (either `Attendance Theory Cont.` or `Attendance Practical Cont.`).</label><br/> 
   <label>* Select a Base Mark For Attendance ("4"/ "8"/ "10").</label><br/> 
   <label>* Type total number of "Working Days" of selected semester. (i.e: 125/130).</label><br/> 
   <label>* Now Press "Next" button(student lists will arrived on the base of selected combination with No. of working days)</label><br/> <br/><br/>

	HOW TO SUBMIT FINAL MARKS<br/>
  	---------------------------------------------<br/><br/>
   <label>* After clicking the "Next" button student lists will arrive.</label><br/>
   <label>* Type Number of Days he/she attend in the class in Selected Semester. Then "Marks(%)" and "Obtained Marks" will calculate automatically.</label><br/>
   <label>* Finally click "Submit Marks" button to Enter Attendance Marks.</label><br/>
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['t'])==1){ if(isset($msg)){ echo "<span style='color:#00CC00; font-weight:bold;'>$msg</span>";} ?><?php } if(isset($_GET['t'])==0){ if(isset($msg)){ echo "<span style='color:#FF9900; font-weight:bold;'>$msg</span>"; } } ?></font></p>
 <div class="search-background1">
	...
	<div id="top-search-div"> 
           <div id="content">
		   <label>Submit Student Attendance Marks</label>
		   <div class="input">
		     <label></label>
		     <label></label>
			 <label></label>
	...
	<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" id="stdtbl">
           ...
           <tr>
             <td width="16%" class="style15"><label>Department :</label></td>
             <td width="46%"><label>
             ...
             <td width="16%" class="style15"><div align="right">
               <label>Course Name :</label>
             </div></td>
             <td align="left"><div id="sub"></div></td>
           </tr>
           <tr>
             <td class="style15">Session :</td>
             <td><select name="session" class="style2" id="session" style="font-family: Verdana; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)" >
               ...
               <?php 
				   ...
				   ?>
               <option value="<?php echo $srow['SESSION']; ?>"><?php echo $srow['SESSION'];//"20".substr_replace($srow['SESSION'],'-20',-2,-2); ?></option>
               <?php } ?>
             </select></td>
             <td width="13%" class="style15"><div align="right">Semester :</div></td>
             <td width="25%"><select name="semester" id="semester" style="font-family: Verdana; font-size: 8pt;padding:3px; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)">
           ...
           <tr>
             <td class="style15">Base Marks :</td>
             <td><select name="bm" class="style4" id="mb" style="font-family: Verdana; width:50px; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)"  >
             ...
             <td><select name="examtype" class="style4" id="examtype" style="font-family: Verdana; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)"  >
                 <option value="Attendance Theory Cont">Attendance Theory Cont</option>
                 <option value="Attendance Practical Cont">Attendance Practical Cont</option>
             </select></td>
             </tr>
           <tr>
             <td class="style15">Working Days  :</td>
             <td ><input type="text" id="twork" name="twork" style="width:60px; " /></td>
             <td class="style15"><div align="right">Section :</div></td>
             <td><span class="style4">
               <select name="section" class="style2" id="select" style="font-family: Verdana; width:80px; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)" >
                 <option>Section</option>
                 <?php 
					$vss="SELECT distinct  a.section  FROM tbl_assignfaculty a INNER JOIN tbl_faculty f ON a.facultyid = f.id INNER JOIN tbl_department d ON a.deptid = d.id INNER JOIN tbl_courses c ON a.courseid = c.id INNER JOIN tbl_semester s ON a.semesterid = s.id WHERE f.storedstatus<>'D' and f.facultyid='$_SESSION[userid]'  and a.attndstatus='0'";
  					$qss=$myDb->select($vss);
				   while($serow=$myDb->get_row($qss,'MYSQL_ASSOC')){
					
				   ?>
                 <option value="<?php echo trim($serow['section']); ?>"><?php echo trim($serow['section']);//echo "20".substr_replace($srow['SESSION'],'-20',-2,-2); ?></option>
                 <?php } ?>
               </select>
               <input type="button" name="submit" id="sbt"  value="Saerch" style="color: #999999; font-size: 8pt; font-family: Verdana; border: 1px solid #C0C0C0; background-color: #D9F0FB" />
</span></td>
             </tr>
           <tr>
             <td colspan="4">&nbsp;</td>
