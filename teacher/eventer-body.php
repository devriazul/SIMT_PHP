	   ...
	   <div align="left"><table width="100" background="" border="0" cellspacing="0" cellpadding="0" >
          <tr>
            
          </tr>
        </table>
</div>
<div  style="width: 80%; height: 491px;" id="eventer-events-calendar-container"  >
            ...
            if ($m >= ($eventerCalendarVars->weekStartDayID + 1) && $m <= $eventerCalendarVars->numDaysInMonth + $eventerCalendarVars->weekStartDayID) {
                ...
                $dateBoxHTML = '<li class="date-box'. $cssClasses .' date-box-row-'. ($eventer_row + 1) .'" id="date-box-'. $m .'">';
                    ...
                    if ($eventerCalendarOptions->repeatEvents) {
						...
						$query = "SELECT * FROM `eventer_eventshces` WHERE start_date<='$startDate' AND end_date>='$startDate' AND section = 'Holiday Calendar' UNION SELECT * FROM `eventer_eventshces` WHERE  start_date='$startDate' AND section = 'Holiday Calendar'";
                    }
                    else {
                        $query = "SELECT * from eventer_eventshces WHERE start_date='$startDate' AND status='1' AND section = 'Holiday Calendar'";
                    }
