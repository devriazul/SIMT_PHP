...
{ 
  ...
  $vs="SELECT f.id, f.facultyid, f.password, f.name, f.fname, f.mname, f.sex, f.dob, f.mstatus, f.bloodgroup, f.address,f.designationid, ifnull(f.smob,0) as smob, ifnull(f.pfob,0) as pfob, d.name as designation, f.joiningdate, f.deptid, dp.name as department, f.expartincourse, f.eduqualification, f.expyear, f.expmonth, f.contactno, f.type, f.payscaleid, p.name as payscale FROM `tbl_faculty` f inner join tbl_designation d on f.designationid=d.id inner join tbl_department dp on f.deptid=dp.id inner join tbl_payscale p on f.payscaleid=p.id WHERE f.storedstatus<>'D' and f.facultyid='$_SESSION[userid]'";
  $r=$myDb->select($vs);
  $row=$myDb->get_row($r,'MYSQL_ASSOC');

  $vsu="SELECT ifnull(SUM(securitymoney),0) as securitymoney, ifnull(SUM(pfundamount),0) as pfundamount FROM `tbl_employeesalary` WHERE empid='$_SESSION[userid]'";
  $ru=$myDb->select($vsu);
  $rowu=$myDb->get_row($ru,'MYSQL_ASSOC');
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['msg'])){ echo $_GET['msg'];  }?></font></p>
		<form name="MyForm" autocomplete="off" action="#" method="post">
          <div align="center">          <table width="91%" border="0" align="center" cellpadding="0" cellspacing="2" id="stdtbl">
            ...
            <tr bgcolor="#DFF4FF">
              <td height="32" colspan="4" class="gridTblHead style2">Salary Structure </td>
              </tr>
            <tr>
            ...
            <tr bgcolor="#DFF4FF">
              <td height="40" colspan="4" class="gridTblHead style2">Other Information</td>
              </tr>
            <tr bgcolor="#F4F4FF">
            ...
            <tr bgcolor="#F4F4FF">
              ...
              <td height="20"><span style="font-family: Verdana; font-size: 8pt; font-weight:bold;"><a target="_blank" href="reportsecuritymoneydetails.php?efid=<?php echo $row['facultyid']; ?>"><?php echo $row['smob']+$rowu['securitymoney']." BDT"; ?></a></span></td>
              <td><span class="style2">Provident Fund  :</span></td>
              <td><span style="font-family: Verdana; font-size: 8pt; font-weight:bold;"><a target="_blank" href="reportprovidentfunddetails.php?efid=<?php echo $row['facultyid']; ?>"><?php echo $row['pfob']+$rowu['pfundamount']." BDT"; ?></a></span></td>
            </tr>
            <tr bgcolor="#F4F4FF">
