...
//getting number of rows and calculating no of pages
  if(isset($_POST['searchid'])){ 
    $sql = "select * from  tbl_accdtl WHERE flname LIKE '%$_POST[searchid]%' and storedstatus<>'D'";  
	$rsd = mysql_query($sql);
    $count = mysql_num_rows($rsd);
    $pages = ceil($count/$per_page);

  }else{
    $sql = "select * from  tbl_accdtl where storedstatus<>'D'";
	$rsd = mysql_query($sql);
    $count = mysql_num_rows($rsd);
    $pages = ceil($count/$per_page);
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><div align="center"><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['t'])){ ?><span style="color:#FF6600; font-weight:bold;"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }?></span><?php } ?></font></div></font></p>
		...
		<div id="top-search-div"> 
		   ...
		   <form method="post" autocomplete="off" action="macclevel_search.php">
			 ...
			 <label><a href="add_macclevelnew.php"><input type="button" name="addbtn" id="submit-btn" value="Add New" /></a></label>
		   </form>
		   </div>
		</div>
		</div>
