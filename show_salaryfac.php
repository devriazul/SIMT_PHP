...
{ 
	...
	{
		$q=$_GET['q'];
		//echo "select ps.basicpay from tbl_faculty f inner join `tbl_payscale` ps on f.payscaleid=ps.id where f.payscaleid='$q'";
		//$s_scat=mysql_query("select ps.basicpay from tbl_faculty f inner join `tbl_payscale` ps on f.payscaleid=ps.id where f.payscaleid='$q'") or die(mysql_error());
