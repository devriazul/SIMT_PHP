...
{ 
  if($_SESSION['userid']){ 
	...
	/*$bki=$myDb->select("SELECT ifnull(count(irstatus),0) totalissue,(SELECT count(`return`)  from 
						...
						AND irstatus='ISSUE'");
	*/
	$bki=$myDb->select("SELECT ifnull(count(irstatus),0) totalissue
	                    FROM tbl_bookissue
						WHERE courseid='$crsf[id]'
						AND irstatus='ISSUE'");
	$bkif=$myDb->get_row($bki,'MYSQL_ASSOC');	
	if(!empty($stcf['stdid'])){
		$avi=(($bkqf['totalbook'])-($bkif['totalissue']));
		if($avi<=0){	/* if less than 0 stock empty*/
				...
				$tbq=$myDb->select("select ifnull(count(*),0) totalbook from tbl_bookissue WHERE stdid='$stcf[stdid]' and irstatus='ISSUE' and `return`<>'R'");
				$tbf=$myDb->get_row($tbq,'MYSQL_ASSOC');
				
				$isb=$myDb->select("SELECT*,DATEDIFF(curdate(),returndate) as dated,curdate()  as currentdate 
									FROM tbl_bookissue WHERE stdid='$stcf[stdid]' and courseid='$courseid' and irstatus='ISSUE' and `return`<>'R'");
				$isbf=$myDb->get_row($isb,'MYSQL_ASSOC');
				
				if(empty($isbf['courseid'])){ /* if not set courseid that means you can take the course it is available for you */
										...
										VALUES('$courseid','$deptid','$stcf[stdid]','$session','$issue','$lsf[rdate]','ISSUE','$bookid','$semesterid','$opby','I')";
