...
/*
	...
	$pdf->Cell(260, -320, 'PART TIME EMPLOYEE SALARY FOR THE MONTH OF: '.$_POST['smonth'].', '.$_POST['syear'],0,'C',0);
...
Salary Advice from SIMT A/C # 4022-792721-000 for the month of '.$_POST['smonth'].', '. $_POST['syear'].'. Please make the pay transfer from above A/C no to the below mentioned A/C no towards employee salaries:';
    // Times 12
...
$pdf->SetX(15);
		$pdf->Cell(15, 6, 'SL No.', 1, 0, 'L', 1);
		$pdf->Cell(80, 6, 'Name of the Employee(s)', 1, 0, 'L', 1);
		$pdf->Cell(50, 6, 'Bank Account No', 1, 0, 'L', 1);
		$pdf->Cell(40, 6, 'Amount (Tk.)', 1, 0, 'R', 1);
...
//---Pervious
//$result=mysql_query("SELECT es.*,sf.bankaccno as BankAccNo  FROM tbl_employeesalary es inner join vw_allstaff sf on es.empid=sf.StaffID WHERE monthname='$_POST[smonth]' and yearname='$_POST[syear]' and sf.bankaccno<>'' and sf.bankaccno not like 'cash%' Order By es.id");
$result=mysql_query("SELECT *  FROM tbl_employeesalary   WHERE monthname='$_POST[smonth]' and yearname='$_POST[syear]' and bankaccno<>'' and bankaccno not like 'cash%' Order By id");
...
while($row = mysql_fetch_array($result))
{
	...
	$pdf->SetFont('Times', '', 10);
		...
		$pdf->Cell(15, 6, $SL, 1, 0, 'L', 1);
		$pdf->Cell(80, 6, $empname, 1, 0, 'L', 1);
		$pdf->Cell(50, 6, $bankaccno, 1, 0, 'L', 1);
		$pdf->Cell(40, 6, $netpay, 1, 0, 'R', 1);
	...
	$resulttr=mysql_query("Select ifnull(sum(netpay),0) as totalamount FROM tbl_employeesalary  WHERE monthname='$_POST[smonth]' and yearname='$_POST[syear]' and bankaccno not like 'cash%'");
	...
	$pdf->Cell(145, 6, 'Total Amount (Tk.) =', 1, 0, 'R', 1);
    $pdf->Cell(40, 6, number_format($trow['totalamount'],2), 1, 0, 'R', 1);
    //$pdf->Cell(35, 6, '', 1, 0, 'L', 1);
