...
<?php 
		  $searchid=!empty($_POST['searchid'])?$_POST['searchid']:'';
  		  if(!empty($searchid)){
			  $sdq="SELECT id, stdid as StudID, stdname as Name, department as Department, 
					session as Session, cgpa as CGPA, passingyear as PassingYear 
					FROM tbl_stdcv 
					WHERE  storedstatus <>'D' 
					AND stdid LIKE '$searchid%'
					order by id desc 
...
<?php 
 /* --------------------------------------------- */
 $msg='';
$query_pag_num = "SELECT COUNT(*) AS count FROM tbl_stdcv";
