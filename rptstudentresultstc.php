...
//Position at 1.5 cm from bottom
	...
	$pdf->Cell(80, 10, 'Student Result Report (Theory Contineous)',0,0,'C',0);
...
$pdf->Cell(50, 6, 'Student Name', 1, 0, 'L', 1);
$pdf->Cell(20, 6, 'B.RollNo', 1, 0, 'C', 1);
$pdf->Cell(15, 6, 'T.Marks', 1, 0, 'C', 1);
$pdf->Cell(20, 6, 'Class Test', 1, 0, 'C', 1);
$pdf->Cell(20, 6, 'Quiz Test', 1, 0, 'C', 1);
$pdf->Cell(30, 6, 'Attandence', 1, 0, 'C', 1);
...
$crs="SELECT mf.stdid FROM tbl_marksentryfinal mf inner join tbl_stdinfo s on mf.stdid=s.stdid WHERE mf.deptid='$_POST[deptid]' and mf.courseid= '$_POST[courseid]' and mf.semesterid='$_POST[semesterid]' and mf.session='$_POST[session]' and mf.year='$_POST[eyear]' order by s.boardrollno ASC";
$crq=$myDb->select($crs); 
...
{
//$count=0;
	 $query="SELECT distinct m.stdid, s.stdname, s.boardrollno, c.coursecode, c.coursename, m.examname, m.session, m.deptid, m.courseid, m.year, m.semesterid, c.cont_assess_t, (c.cont_assess_t+c.f_exam_t+c.cont_assess_p+c.f_exam_p) as TotalMarks,
						(select classtestmarks from tbl_marksentryfinal WHERE deptid='$_POST[deptid]' and courseid= '$_POST[courseid]' and semesterid='$_POST[semesterid]' and session='$_POST[session]' and year='$_POST[eyear]'  and stdid='$crsr[stdid]') as classtest,
						(select quiztestmarks from tbl_marksentryfinal WHERE deptid='$_POST[deptid]' and courseid= '$_POST[courseid]' and semesterid='$_POST[semesterid]' and session='$_POST[session]' and year='$_POST[eyear]' and stdid='$crsr[stdid]') as quiztest,						
						(select `attendancemarks` from tbl_marksentryfinal WHERE deptid='$_POST[deptid]' and courseid= '$_POST[courseid]' and semesterid='$_POST[semesterid]' and session='$_POST[session]' and year='$_POST[eyear]' and stdid='$crsr[stdid]') as attndmarks
...
//echo $row['TotalMarks']; 
    ...
    $totalmarkstc = $row['cont_assess_t'];
	$classtest = $row['classtest'];
    $quiztest = $row['quiztest'];
    $attndtc = $row['attndmarks'];
    $totalom = $classtest + $quiztest + $attndtc;
    ...
    $pdf->Cell(50, 6, $stdname, 1, 0, 'L', 1);
    $pdf->Cell(20, 6, $brollno, 1, 0, 'C', 1);
    $pdf->Cell(15, 6, $totalmarkstc, 1, 0, 'C', 1);
    $pdf->Cell(20, 6, $classtest, 1, 0, 'C', 1);
    $pdf->Cell(20, 6, $quiztest, 1, 0, 'C', 1);
    $pdf->Cell(30, 6, $attndtc, 1, 0, 'C', 1);
