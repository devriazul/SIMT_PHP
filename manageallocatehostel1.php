...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }?></font></p>
	 ...
	 $sdq="SELECT s.id, s.stdid as StudentId, s.stdname AS Name, b.batchname AS Batch, d.name AS Department FROM  `tbl_stdinfo` s inner join `tbl_batch` b on s.batch=b.id inner join `tbl_department` d on s.deptname=d.id WHERE s.storedstatus <>  'D' and s.stdid<>'' AND s.stdname ='$searchid' OR s.exstid = '$searchid'
		  UNION
		  SELECT s.id, s.exstid as StudentId, s.stdname AS Name, b.batchname AS Batch, d.name AS Department FROM  `tbl_stdinfo` s inner join `tbl_batch` b on s.batch=b.id inner join `tbl_department` d on s.deptname=d.id WHERE s.storedstatus <>  'D' and s.exstid<>''
		  AND s.stdid ='$searchid' OR s.exstid = '$searchid' ";
