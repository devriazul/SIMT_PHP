...
{ 
  ...
  $vs="Select af.id, f.id as facultyid, f.name as FacultyName, d.id as deptid, d.name as DepartmentName, c.id as courseid, c.coursename as SubjectName, af.semesterid as semesterid, s.name as semestername, af.session, af.section, af.year from tbl_assignfaculty af inner join tbl_faculty f on af.facultyid=f.id inner join tbl_department d on af.deptid=d.id inner join tbl_courses c on af.courseid=c.id inner join tbl_semester s on af.semesterid=s.id WHERE af.storedstatus<>'D' and af.id='$id'
 order by id";
  $r=$myDb->select($vs);
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['msg'])){ echo $_GET['msg'];  }?></font></p>
		<form name="MyForm" action="ed_courseassigntofaculty.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data" onsubmit="Javascript:return checkstaffdata();">
          ...
          <table width="70%" border="0" align="center" cellpadding="0" cellspacing="5" id="stdtbl">
            ...
            <tr>
              <td height="20" colspan="2" class="style2" style="padding:3px; border-bottom:1px solid #CCCCCC;">EDIT SUBJECT ASSIGN TO FACULTY (<span class="stars">*</span>Mandatory Field) </span></td>
              </tr>
            <tr bgcolor="#F3F3F3">
              <td height="18" colspan="2" class="style2">&nbsp;</td>
              </tr>
            <tr>
              <td width="31%" height="20" class="style2">Faculty Name :<span class="stars">*</span></td>
              <td width="69%" height="20"><select name="facultyid" id="facultyid" onkeypress="return handleEnter(this, event)">
                <option selected="selected" value="<?php echo $row['facultyid']; ?>"><?php echo $row['FacultyName']; ?></option>
                <?php $hq=$myDb->select("select id,name from tbl_faculty");
            ...
            <tr>
              <td><span class="style2">Section :<span class="stars">*</span> </span></td>
              <td><select name="section" class="style2" id="section" style="font-family: Verdana; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)" onchange="showSelected();">
