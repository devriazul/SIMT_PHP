...
{ 
  	if($_SESSION['userid'])
	{
		$chka="SELECT*FROM  tbl_accdtl WHERE flname='leaveapplicationnew.php' AND userid='$_SESSION[userid]'";
...
<script language="javascript">
 ...
 $(document).ready(function(event){
   $('.sbmt').click(function(){
	 ...
	 */
	 if($('#applyfor').val()==''){
	   alert("Please select Leave Type you want to aaply.");
	   $('#applyfor').focus();
	   $('.ui-state-default').css({'visibility':'collapse'});
	   return false;
	 }

	 if($('#attnto').val()==' Select Attn to'){
	   alert("Please select to whom you want to attn.");
	   $('#attnto').focus();
	   $('.ui-state-default').css({'visibility':'collapse'});
	   return false;
	 }
      ...
      <form name="MyForm" id="frm" autocomplete="off"  method="post" >
        <table width="80%" border="0" align="center" cellpadding="0" cellspacing="2" id="stdtbl">
          ...
          <?php //$empid=$_GET['empid'];
  				$vs="SELECT s.*, d.name as Designation FROM tbl_staffinfo s inner join tbl_designation d on s.designationid=d.id WHERE s.sid='$_SESSION[userid]' and s.storedstatus<>'D'";
  				$r=$myDb->select($vs);
  				$row=$myDb->get_row($r,'MYSQL_ASSOC'); //echo $row['Designation']; exit;
				if(($row['clstatus']==1) || ($row['slstatus']==1) || ($row['alstatus']==1) || ($_SESSION['userid']=="administrator"))
				{
				?>
          <tr>
          ...
          <tr>
            <td width="47%" align="center" height="20" class="style2"><table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" id="tblleft">
                <tr bgcolor="#F3F3F3">
...
:                    </td>
                ...
                <tr>
                  ...
                  <td width="33%" class="style2"><span style="font-weight:normal; font-size:12px; "><?php echo $row['sid'];?></span>
                ...
                <tr>
                  ...
                  <td height="20"><select name="applyfor" id="applyfor" onkeypress="return handleEnter(this, event)">
                      ...
                      <?php $hq=$myDb->select("select ' Select Type' as name, '' as code  UNION select name, code from tbl_leave Where storedstatus<>'D'");
				   while($hrow=$myDb->get_row($hq,'MYSQL_ASSOC')){
				   ?>
                      <option value="<?php echo $hrow['code']; ?>"><?php echo $hrow['name']; ?></option>
                      <?php } ?>
                    </select>
                      <input name="td" type="hidden" id="td" style="width: 80px" /></td>
                </tr>
                <tr bgcolor="#F5F5F5">
                ...
                <tr>
                  ...
                  <td height="20" colspan="4"  class="style2" ><span>
                    <select name="attnto" id="attnto" style="width:320px;"   onkeypress="return handleEnter(this, event)">
                      <?php $hq=$myDb->select("Select '' as id, ' Select Attn to' as name, '' as dname UNION select s.id,s.name, d.name as dname from tbl_staffinfo s inner join tbl_designation d on s.designationid=d.id Where s.designationid in(1,6,3) and s.storedstatus<>'D' and s.jobstatus='Active'");
				   while($hrow=$myDb->get_row($hq,'MYSQL_ASSOC')){
				   ?>
                ...
                <tr bgcolor="#F5F5F5">
                  <td height="30" colspan="6" class="style2"><p style="font-weight:bold; font-size:10px; color:#FF6600; ">If you are not a Faculty (Teacher) please ignore this section. During this period my classes has been arranged as follows: </p>                    </td>
                </tr>
                <tr>
