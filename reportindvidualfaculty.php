...
{ 
  if($_SESSION['userid']){
	$fid=$_GET['facultyid'];
  	...
  	$vsm="SELECT ifnull(SUM(securitymoney),0) as SecurityMoney, ifnull(SUM(pfundamount),0) as PFundAmount FROM `tbl_employeesalary` WHERE storedstatus<>'D' AND empid='$fid'";
  	$rm=$myDb->select($vsm);
