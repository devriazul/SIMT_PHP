...
<?php
	...
	if(isset($_POST['Submit']))  {
		...
		$query=" UPDATE `eventer_events` SET `title` =  '$title', `start_date` = '$start_date', `end_date` = '$end_date', `start_time` = '$start_time', `end_time` = '$end_time', `description` = '$description', `venue` = '$venue', `image_alignment` = '$image_alignment', `link` = '$link' , `status` = '$status',`departmentid`='$departmentid',`session`='$session',`semester`='$semester',`coursecode`='$coursecode',`dayname`='$day',`facultyid`='$facultyid'";
...
<tr>
            ...
            <tr>
			  ...
			  <select name="session" id="session" style="font-family: Verdana; font-size: 10pt; border: 1px solid #3399FF;height:30px; padding:5px;" onkeypress="return handleEnter(this, event)">
            ...
            <tr>
			  ...
			  <div id="clist" style="display:none"></div>			  </td>
            </tr>              
            ...
            <tr>
			  ...
			  <div id="fcid">
			    ...
			    <option value="">Select</option>
				<?php $dq=mysql_query("SELECT * FROM tbl_faculty WHERE facultyid='$row[facultyid]' and storedstatus IN('I','U') order by id desc") or die(mysql_error());
					  while($drow=mysql_fetch_array($dq)){
				?>
				<option selected="selected" value="<?php echo $drow['facultyid']; ?>"><?php echo $drow['facultyid']."(".$drow['name'].")"; ?></option>
				<?php } ?>	   
              </select>			  </div>
			  <div id="flist" style="display:none"></div>			  </td>
            </tr>			
