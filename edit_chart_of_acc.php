...
{ 
  	if($_SESSION['userid'])
	{
		$id=mysql_real_escape_string($_GET['id']);
	  	$eqry="SELECT id, accno, accname, type, (SELECT parentid FROM `tbl_accchart` WHERE id='$id') as ParentId, (SELECT accname FROM tbl_accchart WHERE id in(SELECT parentid FROM `tbl_accchart` WHERE id='$id')) as ParentHead, display FROM tbl_accchart WHERE id='$id'";
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php echo $_GET['msg'];?></font></p>
		<form id="form1" name="form1" method="post" action="ed_chart_of_acc.php?id=<?php echo $id;?>">
          <div align="center"><br />
          <table width="70%" border="0" align="center" cellpadding="0" cellspacing="5" id="stdtbl">

            <tr>
              <td width="17%" class="style2">Account Code</td>
              <td width="3%" height="20" class="style2"><div align="center">: </div></td>
              <td width="80%" height="20"><div align="left">
                <input name="acccode" type="text" id="acccode" style="font-family: Verdana; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)" size="20" value="<?php echo $egr['accno'];?>" />
              </div></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td height="20" ><div align="left">
                <select name="parenthead" id="parenthead" class="style2" onkeypress="return handleEnter(this, event)">
					...
					<option value="<?php echo $egr['ParentId']; ?>" selected="selected"><?php echo $egr['ParentHead']; ?></option>
				  
                    <?php 
			  	$cat=mysql_query("select id, display from tbl_accchart") or die(mysql_error());
	 			while($cfetch=mysql_fetch_array($cat)){
	 			?>
                    <option value="<?php echo $cfetch['id']; ?>"><?php echo $cfetch['display']; ?></option>
                    <?php } ?>
                </select>
              </div></td>
            </tr>
