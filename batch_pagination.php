...
<?php 
     ...
     $sdq="SELECT b.id id, b.batchname AS batchname, d.name AS name, b.session AS SESSION
            FROM tbl_batch b
            INNER JOIN tbl_department d ON b.depcode = d.id 
			WHERE d.name LIKE '$searchid%'
...
<?php 
 /* --------------------------------------------- */
 $msg='';
$query_pag_num = "SELECT COUNT(*) AS count FROM tbl_batch";
