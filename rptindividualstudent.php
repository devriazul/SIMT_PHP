...
{ 
  if($_SESSION['userid']){
  $chka="SELECT*FROM  tbl_accdtl WHERE flname='rptindividualstudent.php' AND userid='$_SESSION[userid]'";
...
$().ready(function() {
	$("#searchid").autocomplete("search_student.php", {
		width: 260,
		matchContains: true,
		//mustMatch: true,
		//minChars: 0,
...
$(document).ready(function(){
    	$('#searchid').keyup(function(e){
	       ...
	       $.post('showstudent.php',arr,function(data){
	...
	$('#searchidBR').keyup(function(e){
	       var arr=$('#sfrm').serializeArray();
	       $.post('showstudentBR.php',arr,function(data){
  ...
  <?php } ?>
		...
		<div id="top-search-div"> 
		   ...
		   <form id="sfrm" method="post" autocomplete="off" action="">
			 ...
			 <label><input type="text" id="searchidBR" name="searchidBR" placeholder="Search by Board Roll" /></label>
		   </form>
		   </div>
		</div>
		</div>
