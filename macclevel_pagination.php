...
<?php 
    ...
    if(!empty($searchid)){
			 ...
			 $sdq="select id,userid as 'User ID',flname as 'File Name',ins as 'Insert',upd as 'Update',delt as 'Delete' from tbl_accdtl where storedstatus<>'D' order by id desc limit $start,$per_page";
			 $sdep=$myDb->dump_query($sdq,'edit_macclevel.php','delmacclevel.php',$car['upd'],$car['delt']);
    }
...
<?php 
 /* --------------------------------------------- */
 $msg='';
$query_pag_num = "SELECT COUNT(*) AS count FROM tbl_accdtl where storedstatus<>'D'";
