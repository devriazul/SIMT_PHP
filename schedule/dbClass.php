...
{ 
   ...
   public function connect($host='',$user='',$pw='',$db='',$persistant=true)
   {
	...
	if($persistant)
	  $this->db_link=mysql_pconnect($this->host,$this->user,'root') or die("database connection has gone away");
	else
  ...
  public function dump_query($sql,$filename,$delname,$dupd='',$ddlt='')
  {
	...
	if(!$r)
	  return false;
    echo "<table width=\"97%\" class=\"dmpquery\" style=\"border:1px solid #000000;padding:4px;\" align=\"center\" cellpadding=\"1\" cellspacing=\"0\">\n";
	$i=0;
	while($row=mysql_fetch_assoc($r))
	{
	   if($i==0)
	   {
		 ...
		 if(($dupd=="y")){ 
			 if(!empty($row['intervalName'])||!empty($row['ordernum'])){
						 echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000;padding:4px;\"> 	  
				<a href=\"#\" name=\"edt\" dorder=\"$row[orderid]\" alt=\"$row[id]\" yrpart=\"$row[yrpart]\" intname=\"$row[intervalName]\" ordernum=\"$row[ordernum]\" >EDIT</a></TD>\n";
			 }else if(!empty($row['dyname'])){
						 echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000;padding:4px;\"> 	  
				<a href=\"#\" name=\"edt\" dyname=\"$row[dyname]\" dorder=\"$row[orderid]\" alt=\"$row[id]\" >EDIT</a></TD>\n";
			 }else if(!empty($row['venuname'])||!empty($row['roomno'])){
						 echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000;padding:4px;\"> 	  
				<a href=\"#\" name=\"edt\" dorder=\"$row[orderid]\" alt=\"$row[id]\" venuname=\"$row[venuname]\" roomno=\"$row[roomno]\" >EDIT</a></TD>\n";
			 }else if(!empty($row['Department ID'])||!empty($row['Alias'])){
						 echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000;padding:4px;\"> 	  
				<a href=\"#\" name=\"edt\" dorder=\"$row[OrderID]\" alt=\"$row[id]\" deptid=\"$row[DepartmentID]\" alias=\"$row[Alias]\" >EDIT</a></TD>\n";
			 }
         }
		 if(($ddlt=="y")){ 
		   if(isset($row['id'])){
              echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000\"> 	  
                   <a href=\"#\" name=\"dlt\" alt=\"$row[id]\" >DELETE</a></TD>\n";
		   }		   
         }
  ...
  public function dump_queryP($sql,$filename,$delname,$dupd='',$ddlt='',$dprv='')
  {
    $r=$this->select($sql);
	if(!$r)
	  return false;
...
<a href=\"$filename?id=$row[id]\">EDIT</a></TD>\n";
		 ...
		 if(($dupd=="y")){ 
         echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000;padding:4px;\"> 	  
<a href=\"$filename?id=$row[id]\">PREVIEW</a></TD>\n";
         }
