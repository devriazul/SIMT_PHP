...
{ 
  ...
  if($car['ins']=="y"){
	...
	if(($htrow['psid']==$htrow['payscaleid'])&&($htrow['psid']!="")&&($htrow['payscaleid']!="")){
	   ...
	   $query="INSERT INTO `tbl_logdetails` (`formname`, `tblname`, `tblid`, `opby`, `opdate`, `optime`) VALUES ('Hostelseat Details', 'tbl_hostelseatdetails', '$id',  '$_SESSION[userid]','$opdate',CURTIME())";
	   $myDb->insert_sql($query);

	   
	   
	   $qup="DELETE From tbl_hostelseatdetails WHERE `id`='$id'";
	   $upd=$myDb->update_sql($qup);
	   $msg="Data deleted successfully";
       header("Location:managehostelseatdetails.php?msg=$msg");
	//}   
   }else{
