...
{ 
  ...
  $id=mysql_real_escape_string($_GET['id']);
   $chkstd=" SELECT * From tbl_parttimeemployeesalary Where id='$id'";
  ...
  /*   if(($stdq['crsid']==$stdq['coursename'])&&($stdq['crsid']!="")&&($stdq['coursename']!="")){  
       ...
       header("Location:managecourseinformation.html?msg=$msg");	
	    ...
	    $query="INSERT INTO `tbl_logdetails` (`formname`, `tblname`, `tblid`, `opby`, `opdate`, `optime`) VALUES ('Add Employee Salary', 'tbl_parttimeemployeesalary', '$id',  '$_SESSION[userid]','$opdate',CURTIME())";
		$myDb->insert_sql($query);
				
	   $qup="Delete From tbl_parttimeemployeesalary WHERE `id`='$id'";
	   if($myDb->update_sql($qup))
	   {

			$msg="Data deleted successfully";
          	header("Location:manageptemployeesalary.php?msg=$msg");
	   }else{
	      $msg=$myDb->last_error;
		  echo $msg;
	   }	  	  	
