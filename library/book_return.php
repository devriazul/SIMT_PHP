...
$(document).ready(function(){	 
  ...
  $('#stdid1<?php echo $_GET['rowid']; ?>').change(function(){
  ...
  $('#bookentry1<?php echo $_GET['rowid']; ?>').click(function(){
    var semesterid=$('#semesterid1<?php echo $_GET['rowid']; ?>').val();
	var session=$('#session1<?php echo $_GET['rowid']; ?>').val();
      if($('#deptid1<?php echo $_GET['rowid']; ?>').val()==""){
	     alert('Department ID can not left empty');
		 $('#deptid1<?php echo $_GET['rowid']; ?>').focus();
		 return flase;
	  }	 
	  if($('#stdid1<?php echo $_GET['rowid']; ?>').val()==""){
	     alert('Student ID can not left empty');
		 $('#stdid1<?php echo $_GET['rowid']; ?>').focus();
		 return flase;
	  }	 
	  /*if($('#semesterid1<?php echo $_GET['rowid']; ?>').val()==""){
	     alert('Semester ID can not left empty');
		 $('#semesterid1<?php echo $_GET['rowid']; ?>').focus();
		 return flase;
	  }  
	  if($('#session1<?php echo $_GET['rowid']; ?>').val()==""){
	     alert('Session can not left empty');
		 $('#session1<?php echo $_GET['rowid']; ?>').focus();
		 return flase;
	  } 
	  */     
	  if($('#issuee1<?php echo $_GET['rowid']; ?>').val()==""){
	     alert('Issue date can not left empty');
		 $('#issuee1<?php echo $_GET['rowid']; ?>').focus();
		 return flase;
	  }  
	  var arr=$('#bisu1<?php echo $_GET['rowid']; ?>').serializeArray();
	  $.post('issue_return.php?semesterid='+semesterid+"&session="+session+"&rowid=<?php echo $_GET['rowid']; ?>",arr,function(data){
  ...
  $('#issuee1<?php echo $_GET['rowid']; ?>').blur(function(){
    var cdate='<?php echo date("Y-m-d"); ?>';
	if($('#issuee1<?php echo $_GET['rowid']; ?>').val()<cdate){
...
<script language="javascript">
$().ready(function() {
		$("stdid1<?php echo $_GET['rowid']; ?>").autocomplete("search_student.php", {
				width: 260,
				matchContains: true,
				//mustMatch: true,
				//minChars: 0,
...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td>Semester<span style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:9px; color:#FF0000" >*</span>
	<input type="hidden" name="bookid" id="bookid<?php echo $bqf['bookid']; ?>" value="<?php echo $bqf['bookid']; ?>" />
	<input type="hidden" name="courseid" id="courseid<?php echo $bqf['courseid']; ?>" value="<?php echo $bqf['courseid']; ?>"/></td>
    <td><select name="semesterid" id="semesterid1<?php echo $_GET['rowid']; ?>" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:12px;" onkeypress="return handleEnter(this, event)" disabled>
  ...
  <tr>
    ...
    <td><select name="session" class="style4" id="session1<?php echo $_GET['rowid']; ?>" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size: 12px;" onkeypress="return handleEnter(this, event)" disabled>
  ...
  <tr>
    ...
    <td>	<input type="date" name="issue" id="issuee1<?php echo $_GET['rowid']; ?>" size="30" value="<?php echo date("Y-m-d"); ?>" onkeypress="return handleEnter(this, event)" min="<?php echo date("Y-m-d"); ?>">
  ...
  <tr>
    <td height="30">&nbsp;</td>
    <td><label>
      <input type="hidden" name="fine" id="fine<?php echo $_GET['rowid']; ?>" />
    </label></td>
  </tr>
  <tr>
    <td height="30">&nbsp;</td>
