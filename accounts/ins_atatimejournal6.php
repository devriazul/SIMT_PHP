...
{ 
  ...
  if($car['ins']=="y"){
  $_SESSION['vdate']=$_POST['voucherdate6']?$_POST['voucherdate6']:date("Y-m-d");
  if(!empty($_POST['msthead6'])&&!empty($_POST['dr6'])){
	 
     $ms=$myDb->select("select*from tbl_accchart where accname like '$_POST[msthead6]%' and id not in(select parentid from tbl_accchart) order by accname");
	 while($msf=$myDb->get_row($ms,'MYSQL_ASSOC')){
		 $jrc=$myDb->select("select*from tbl_2ndjournal where accname='$msf[accname]' and voucher_group='$_POST[msthead6]'");
	   ...
	   //if(empty($jrcf['accname'])){
		...
		$myDb->insert_sql("INSERT INTO `tbl_masterjournal` (`voucherid`, `voucherdate`, `vouchertype`, 
						...
						('$mvidf', '".$_SESSION['vdate']."', 'J', '$_SESSION[userid]', '&nbsp;', '', '', '$msf[id]', '', '', '$_SESSION[userid]', '".$_SESSION['vdate']."', 'I', 'single', '$_POST[msthead6]')");
		...
		$ins2nd="INSERT INTO tbl_2ndjournal(accno,accname,amountdr,amountcr,voucherid,
										...
										VALUES('$msf[id]','$msf[accname]','$_POST[dr6]',
											   '0','$mvidf','J','&nbsp;',
		...
		while($cheadf=$myDb->get_row($chead,'MYSQL_ASSOC')){				
		    $crv=$_POST["cr6$cheadf[id]"];				
			if(!empty($cheadf['id'])&&$_POST["cr6$cheadf[id]"]>0){			
				$ins2nd="INSERT INTO tbl_2ndjournal(accno,accname,amountdr,amountcr,voucherid,
												vouchertype,paytype,vdate,masteraccno,
												storedstatus,opby,parentid,groupname,multi_single,voucher_group)
