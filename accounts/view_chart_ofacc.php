...
<html>
	...
	$(document).ready(function(){
		...
		$('#searchid').keypress(function(){
		  ...
		  $.post("view_chart_ofacc.php",arr,function(msg){
...
while($row = mysql_fetch_assoc($rsCategories)){ 
	...
	$arrayCategories[$row['id']] = array("parent_id" => $row['parentid'],"acid"=> $row['id'], "name" => $row['accname']);	
...
function createTree($array, $currentParent, $currLevel = 0, $prevLevel = -1) {
	...
	foreach ($array as $categoryId => $category) {
		...
		if ($currentParent == $category['parent_id']) {						
			...
			$acq=mysql_query("select*from tbl_accchart where id='$category[acid]' and id in(select accno from tbl_2ndjournal)") or die(mysql_error());
			$acqf=mysql_fetch_array($acq);
			if(!empty($acqf['id'])){

				echo '<li id="'.$categoryId.'"><span>'.$category['name'].'</span>';
			}else{
				echo '<li id="'.$categoryId.'"><span>'.$category['name'].'</span><img src="images/16-circle-blue-add.png" alt="add subcategory" width="16" height="16" class="add_category"/> ';
			}
...
<tr>
  ...
  <tr>
	...
	<ul class="simpleTree">
		...
		<li id="0" class="root"><span class="style3">Create Account Head </span><img src="images/16-circle-blue-add.png" alt="add subcategory" width="16" height="16" class="add_category"/> 
