...
{ 
  if($_SESSION['userid']){
  $chka="SELECT*FROM  tbl_accdtl WHERE flname='manage_acc_group_head.php' AND userid='$_SESSION[userid]' and storedstatus<>'D'";
...
$().ready(function() {
	$("#searchid").autocomplete("search_acchead.php", {
		width: 260,
		matchContains: true,
		//mustMatch: true,
		//minChars: 0,
 ...
 <script type="text/javascript">
            $(document).ready(function(){				   
                ...
                function loadData(page){
                    ...
                    ({
                        type: "GET",
                        url: "acc_group_head_pagination.php",
                        data: "page="+page,
					  //  data:$("form").serialize(),
				...
				$('#searchid').keyup(function(){
				  ...
				  $.post("acc_group_head_pagination.php?page=1",arr,function(r){
				    $('#container').html(r);
				  });
				});         
            });
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><div align="center"><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['t'])){ ?><span style="color:#FF6600; font-weight:bold;"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }?></span><?php } ?></font></div></font></p>
		...
		<div id="top-search-div"> 
           <div id="content">
		   <div class="input1">
		   <form method="post" name="frmsrch" id="frmsrch" autocomplete="off" action="#">
		     <label>Search By: </label>
			 <label><input type="text" id="searchid" name="searchid" placeholder="Account Name" /></label>
			 <label><input type="button" name="subs" id="submit-btn" value="Search" /></label>
			 <label><a href="add_chart_group.php" name="addbtn" class="submit-btn" style="padding:5px;">Add New</a></label>
		   </form>
		   </div>
		</div>
		</div>
