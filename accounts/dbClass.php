...
<a href=\"$filename?id=$row[id]\">EDIT</a></TD>\n";
  ...
  public function dump_queryFORDELETE($sql,$filename,$delname,$dupd='',$ddlt='')
  {
    $r=$this->select($sql);
	if(!$r)
	  return false;
	...
	while($row=mysql_fetch_assoc($r))
	{
	   if($i==0)
	   {
		 ...
		 if(($dupd=="y")){ 
         echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000;padding:4px;\"> 	  
<a href=\"#\" id=\"$row[id]\" name=\"edt$row[id]\" >EDIT</a></TD>\n";
?>
			<script language="javascript">
			 $(document).ready(function(){
				$('a[name="edt<?php echo $row['id']; ?>"]').click(function(e){
					e.preventDefault();
					var id = $(this).attr('id');
				    $('.accpopup').load("edit_acc_head.php?id="+id).show();
				});
			 });
			</script>
...
<?php
		 ...
		 if(($ddlt=="y")){ 
		   ...
		   ?>
			<script language="javascript">
			 $(document).ready(function(){
				$('a[name="del<?php echo $row['id']; ?>"]').click(function(e){
					e.preventDefault();
					var id = $(this).attr('id');
					$.get("deleteAccChartHead.php?id="+id,function(r){
					  $('#container').html(r);//.load("acchead_pagination.php");
					});
				});
			 });
			</script>
		   ...
		   <?php
              echo "<TD class=\"style4\" BGCOLOR=\"$bg\" style=\"border:1px solid #000000\"> 	  
                   <a href=\"#\" id=\"$row[id]\" name=\"del$row[id]\" onClick=\"return confirm('Are you sure want to delete the record!')\">DELETE</a></TD>\n";
		   }		   
         }
