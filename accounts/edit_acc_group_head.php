...
$().ready(function() {
	$("#searchgroup").autocomplete("search_accgroupnew.php", {
		width: 260,
		matchContains: true,
		//mustMatch: true,
		//minChars: 0,
	...
	$("#searchparent").autocomplete("search_accgroupnew.php", {
		width: 260,
		matchContains: true,
		//mustMatch: true,
		//minChars: 0,
...
<script language="javascript">
 $(document).ready(function(){
   ...
   $('input[name="B1"]').click(function(){
	...
	$.post("ed_acc_group_head.php",arr,function(r){
	  $('#shwrec').html(r);
...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }?></font></p>
		...
		<form id="form1" name="form1" method="post">
          ...
          <table width="70%" border="0" align="center" cellpadding="0" cellspacing="2" id="stdtbl">
            <tr>
              <td height="30" colspan="3" class="style2" style="border-bottom:1px solid #999999; ">MODIFY GROUP HEAD</td>
              </tr>
            <tr>
              <td class="style2">&nbsp;</td>
			...
			<?php $acg = $myDb->select("select id,accname from tbl_accchart where id='$edrecf[groupname]'");
			$acgf = $myDb->get_row($acg,'MYSQL_ASSOC');
			?>
            <tr>
              <td class="style2">Group</td>
              <td height="20" class="style2">:</td>
              <td height="20"><input name="searchgroup" type="text" id="searchgroup" value="<?php echo $acgf['id'].'->'.$acgf['accname']; ?>" placeholder="search account head group" style="font-family: Verdana; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)" size="40" /></td>
            </tr>
			<?php $acp = $myDb->select("select id,accname from tbl_accchart where id='$edrecf[parentid]'");
			$acpf = $myDb->get_row($acp,'MYSQL_ASSOC');
			?>
            <tr>
              <td class="style2">Parent</td>
              <td height="20" class="style2">:</td>
              <td height="20"><input name="searchparent" type="text" id="searchparent" value="<?php echo $acpf['id'].'->'.$acpf['accname']; ?>" placeholder="search account head parent" style="font-family: Verdana; font-size: 8pt; border: 1px solid #3399FF" onkeypress="return handleEnter(this, event)" size="40" /></td>
            </tr>
            <tr>
