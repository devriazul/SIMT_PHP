...
<?php $fdate=mysql_real_escape_string($_POST['fdate']);
    ...
    <tr>
      <td height="30" bgcolor="#0033FF" style="padding-left:5px;padding-top:5px;"><span class="style19">Particulars</span></td>
      <td height="30" bgcolor="#0033FF" style="padding-top:5px;"><span class="style19">Dr</span></td>
      <td height="30" bgcolor="#0033FF" style="padding-top:5px;"><span class="style19">Cr</span></td>
    </tr>
    <?php $bb=0;$cc=0;
    ...
    <?php  
		...
		while($tsf=mysql_fetch_array($ts)){
		  $smq=mysql_query("SELECT accname,groupname chgroup,(sum(amountdr)-sum(amountcr)) balance
		   from tbl_2ndjournal
	...
	?>
	<tr>
      <?php if(($smqf['accname']!="") &&($smqf['chgroup']!=4270)){ ?>
      <td height="30" class="s-header"><?php echo "<span style='padding-left:35px'>".$smqf['accname']."</span></br>"; ?> </td>
      <td height="30" class="s-header"><?php if($smqf['balance']>0){ echo $smqf['balance'];}else{ echo 0; } ?></td>
      <td height="30" class="s-header"><?php if($smqf['balance']<0){ echo (-($smqf['balance'])); }else{ echo 0; } ?></td>
	</tr>
	<?php }else{ ?>
	<tr>  
