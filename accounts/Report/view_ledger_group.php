...
<table width="500" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    ...
    <td height="30" colspan="2" style="border-bottom:1px solid #999999; " align="left"><a href="viewJournalReport.php?voucherid=<?php echo $vchid; ?>" target="new">Report</a></td>
  </tr>
  <tr>
    <td height="30" style="border-bottom:1px solid #999999; ">&nbsp;</td>
    <td height="30" colspan="2" style="border-bottom:1px solid #999999; " align="left">		
	  ...
	  Date
	  <input type="date" name="voucherdate" id="voucherdate<?php echo $scount; ?>" size="30" value="<?php echo $mvchf['voucherdate']; ?>" onkeypress="return handleEnter(this, event)" class="field field_medium" >
</td>
    </tr>
  <tr>
  ...
  <tr><?php if($cheadf['amountdr']>0){ ?>
    ...
    <?php } ?>
	<td><?php if($cheadf['amountdr']>0){ ?><a href="#" tdata="<?php echo $cheadf['id']; ?>" tamount="<?php echo $cheadf['amountdr']; ?>" name="dr<?php echo $cheadf['id']; ?>" style="border: 1px solid #DDD;padding: 1px 10px;text-align: center;text-transform: uppercase;top: 5px;position: relative;width: 100%; background-color:blue; color:#fff; ">Transfer</a><?php } ?></td>
  </tr>
<script language="javascript">
  $(document).ready(function(){
   	$('a[name="dr<?php echo $cheadf['id']; ?>"]').click(function(e){
		e.preventDefault();
		var id = $(this).attr('tdata');
		var amount = $(this).attr('tamount');
		$.get("transfer.php?rd=dr&amount="+amount+"&id="+id,function(r){
			alert(r);
		});
	});
	$('a[name="cr<?php echo $cheadf['id']; ?>"]').click(function(e){
		e.preventDefault();
		var id = $(this).attr('tdata');
		var amount = $(this).attr('tamount');
		$.get("transfer.php?rd=cr&amount="+amount+"&id="+id,function(r){
			alert(r);
		});
	});
	
	/*$('#dr<?php echo $scount; ?>').keyup(function(){
		$('#cr<?php echo $scount; ?>').val($('#dr<?php echo $scount; ?>').val());
	});
	$('#cr<?php echo $scount; ?>').keyup(function(){
		$('#dr<?php echo $scount; ?>').val($('#cr<?php echo $scount; ?>').val());
	});
	*/
  });
</script>
  ...
  <tr><?php if($cheadf['amountcr']>0){ ?>
    ...
    <td><?php if($cheadf['amountcr']>0){ ?><a href="#" tdata="<?php echo $cheadf['id']; ?>" name="cr<?php echo $cheadf['id']; ?>" tamount="<?php echo $cheadf['amountcr']; ?>" style="border: 1px solid #DDD;padding: 1px 10px;text-align: center;text-transform: uppercase;top: 5px;position: relative;width: 100%;background-color:blue; color:#fff;">Transfer</a><?php } ?></td>
