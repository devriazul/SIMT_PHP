...
<script language="javascript">
 $(document).ready(function(){
   ...
   $('input[name="B1"]').click(function(){
	...
	$.post("ins_acc_head.php",arr,function(r){
	  $('#accname').val('');
	  $('#shwrec').html(r);
		...
		<form id="form1" name="form1" method="post">
          ...
          <table width="70%" border="0" align="center" cellpadding="0" cellspacing="2" id="stdtbl">
            ...
            <tr>
              ...
              <td height="20" class="style2">:</td>
              <td height="20"><select name="parentid" id="parentid" class="style2" onkeypress="return handleEnter(this, event)">
                <option selected="selected">Select Parent</option>
				<?php $rt=$myDb->select("SELECT id,accname 
											from tbl_accchart 
											where parentid!=0 
											and id in(select parentid from tbl_accchart) 
											and storedstatus<>'D' 
										 UNION ALL
										 select id,accname
										 from tbl_accchart
										 where id=(select max(id) from tbl_accchart where id not in(select parentid from tbl_accchart))
										 order by id asc 	
											");
            ...
            <tr>
              ...
              <td height="20" class="style2">:</td>
              <td height="20"><select name="groupname" id="groupname" class="style2" onkeypress="return handleEnter(this, event)">
                <option selected="selected">Select Parent</option>
				<?php $rt=$myDb->select("SELECT id,accname from tbl_accchart where (parentid>0 or parentid=0) and groupname=0 and storedstatus<>'D' order by id asc");
				while($rtf=$myDb->get_row($rt,'MYSQL_ASSOC')){
				?>
                <option value="<?php echo $rtf['id']; ?>"><?php echo $rtf['accname']; ?></option>
				<?php } ?>
