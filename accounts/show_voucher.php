	...
	<?php if (!empty($_SESSION['userid'])) { 
		...
		?>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="gridTbl" width="90%">
				...
				//previous code
				//$uac=$myDb->select("SELECT * FROM tbl_masterjournal WHERE opby='$_SESSION[userid]' AND voucherid ='$src' and year(voucherdate)='$year' and monthname(voucherdate)='$month' and multi_single in('single','') order by id desc");
				$uac=$myDb->select("SELECT * FROM tbl_masterjournal WHERE voucherid ='$src'");
				$count12=0;
