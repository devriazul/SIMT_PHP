  ...
  $(document).ready(function(){
			 ...
			 $('#accname_m').keyup(function(){
			   ...
			   // //$.get('accname_bottomhead.php?p='+p,function(result){
				    //$('#bothead').show('fast');
				    //$('#bothead').html(result).show('fast');
					//$('#selectmenu').focus();
				
				});
	       //$("#bothead").fadeIn('fast');
		  // $("#bothead").html("<img src='loader.gif' />");
...
$().ready(function() {
 ...
 <script>
  $(document).ready(function() {
	...
	if( $('#DPC_voucherdate_YYYY-MM-DD').select()){
				var vdate=$("#DPC_voucherdate_YYYY-MM-DD").val();
				$('#amval').load('voucher_nosave.php?vdate='+vdate).fadeIn(300);
				$('#amval').load('voucher_notcomplete.php').fadeIn(300);
	}
	  $("#DPC_voucherdate_YYYY-MM-DD").focus(function(){
	  
		 
			 $("#DPC_voucherdate_YYYY-MM-DD").datepicker();
			 
			 $("a[class=ui-state-default]").click(function(e){
				 e.preventDefault();
				 $('#vouchertype').focus();
				 $('#vouchertype').select();
...
$(document).keyup(function(e) {
  ...
  if (e.keyCode == 16) { $("#DPC_voucherdate_YYYY-MM-DD").focus(); }
...
<div id="wrapper">
    ...
    <div style="float:left;width:420px;height:350px;border:1px solid #CCCCCC;margin-left:30px;">
	...
	<label style="float:left;margin-left:10px;margin-top:10px;">Voucher Date:</label>
		<div style="margin-left:135px;"><input name="voucherdate" type="text" id="DPC_voucherdate_YYYY-MM-DD" onkeypress="return handleEnter(this, event)" class="field field_medium"
			placeholder="Voucher Date" value="<?php if(!empty($_SESSION['vdate'])){ echo $_SESSION['vdate']; }else{ echo date("Y-m-d"); } ?>" />
	</div>		
	...
	<label style="float:left;margin-left:10px;margin-top:10px;">Pay Type:</label>
	  ...
	  <div id="am">
	  <label style="float:left;margin-left:10px;margin-top:10px;">Through Account Name:</label>
  ...
  <div id="mask"></div>
<div id="bothead" style=" position:absolute; background-color:#999999; padding:10px; color:#FFFFFF; left:300px;float:right;width:800px; height:auto; display:none; font-family:'Courier New', Courier, monospace; font-size:14px;">
  ...
  ----------------------------<BR/><BR/>
   <label>2 Types of miss balanced occuured in the voucher</label><br/>
    <ul>
	 <li>1st Voucher miss balance lavel,this types of error occured for Receive and Payment type voucher,You see messege may be like this
	     <ul>
		   <li>A voucher still waiting for saving!you have to save the voucher first then start new one</li>
		 </ul>
	 </li>	
	 <li>2nd Voucher miss balance occured for journal voucher,You see messege may be like this
	   <ul><li>Debit and Credit not level you have to complete the voucher first or delete the voucher from queue</li></ul>
	 </li>
   </ul>    
...
<form method="post" name="savejournal" id="savejournal">
  <tr class="ef">
    <td class="d">
	<textarea name="description" id="description" cols="93" onkeypress="return handleEnter(this, event)" placeholder="Narration for voucher"></textarea>
  </tr>
  <tr>
