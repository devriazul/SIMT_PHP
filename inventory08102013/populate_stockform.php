...
{ 
  ...
  if(isset($_GET['storeid'])){ $storeid=$_GET['storeid']; }else{ $storeid=''; }
   ...
   $porderid=mysql_real_escape_string($_GET['porderid']);
   $req=$myDb->select("select*from tbl_purorder where porderid='$porderid'");
...
<form name="sfrm" id="sfrm" method="post">
  ...
  <tr>
    ...
    <tr>
    <td width="124">Porder ID</td>
    <td width="6">:</td>
    <td width="171"><input type="text" name="porderid" id="porderid" onKeyPress="return handleEnter(this, event)" value="<?php echo $reqf['porderid']; ?>" /></td>
    <td width="116"></td>
    <td width="5">&nbsp;</td>
    <td width="178"><label>
    </label></td>
