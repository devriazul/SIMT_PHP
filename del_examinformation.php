...
{ 
  ...
  if(($car['ins']=="y")||($_SESSION['userid']=="administrator")){
	...
	if(($htrow['psid']==$htrow['payscaleid'])&&($htrow['psid']!="")&&($htrow['payscaleid']!="")){
	   ...
	   $qup="DELETE From tbl_examinitionsettings WHERE `id`='$id'";
	   $upd=$myDb->update_sql($qup);
	   $opdate=date("Y-m-d");
	   $optime=date("H:i:s");
	   $qupld="INSERT into tbl_logdetails (formname,tblname,tblid,opby,opdate,optime) VALUES('del_examinformation.php','tbl_examinitionsettings','$id','$_SESSION[userid]','$opdate','$optime')";
	   $updld=$myDb->update_sql($qupld);
	   
	   $msg="Data deleted successfully";
       header("Location:manageexaminformation.php?msg=$msg");
	//}   
   }else{
