		...
		$TR = count($_POST['stdid']);
  		for ($i=0; $i <$TR; $i++)
  		{
			...
			else if($examname=="Job/Experiment Final")
			{
				...
				//if($myDb->insert_sql($query)){
				$query="Update tbl_marksentryfinal set jobexprfinal='$ctmarks' WHERE stdid='$stdid' and deptid='$deptid' and courseid='$courseid' and session='$session' and year='2013' and semesterid='$semesterid' and opby='$opby'";
				if($myDb->update_sql($query)){
