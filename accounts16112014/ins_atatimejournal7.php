...
{ 
  ...
  if($car['ins']=="y"){
  $_SESSION['vdate']=$_POST['voucherdate7']?$_POST['voucherdate7']:date("Y-m-d");
  if(!empty($_POST['msthead7'])&&!empty($_POST['dr7'])){
	 
     $ms=$myDb->select("select*from tbl_accchart where accname like '$_POST[msthead7]%' and id not in(select parentid from tbl_accchart) order by accname");
	 while($msf=$myDb->get_row($ms,'MYSQL_ASSOC')){
		 $jrc=$myDb->select("select*from tbl_2ndjournal where accname='$msf[accname]' and voucher_group='$_POST[msthead7]'");
	   ...
	   //if(empty($jrcf['accname'])){
		...
		$myDb->insert_sql("INSERT INTO `tbl_masterjournal` (`voucherid`, `voucherdate`, `vouchertype`, 
						...
						('$mvidf', '".$_SESSION['vdate']."', 'J', '$_SESSION[userid]', '&nbsp;', '', '', '$msf[id]', '', '', '$_SESSION[userid]', '".$_SESSION['vdate']."', 'I', 'single', '$_POST[msthead7]')");
		...
		$ins2nd="INSERT INTO tbl_2ndjournal(accno,accname,amountdr,amountcr,voucherid,
										...
										VALUES('$msf[id]','$msf[accname]','$_POST[dr7]',
											   '0','$mvidf','J','&nbsp;',
		...
		while($cheadf=$myDb->get_row($chead,'MYSQL_ASSOC')){				
		    $crv=$_POST["cr7$cheadf[id]"];
			if(!empty($cheadf['id'])&&$_POST["cr7$cheadf[id]"]>0){							
				$ins2nd="INSERT INTO tbl_2ndjournal(accno,accname,amountdr,amountcr,voucherid,
												vouchertype,paytype,vdate,masteraccno,
												storedstatus,opby,parentid,groupname,multi_single,voucher_group)
