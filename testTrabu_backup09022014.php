...
{ 
  ...
  $smqf=$myDb->get_row($smq,'MYSQL_ASSOC');	
	  ...
	  if(isset($_GET['show'])){
		  ...
		  $stqP=$myDb->select("select*from tbl_stdinfo where deptname='$deptid' and semester='$semester' and session='$session' LIMIT $mainSTART,$mainPERPAGE");
	  }	  
...
<?php 	$crs=$myDb->select("SELECT distinct d.id,d.name,c.coursename,c.coursecode,s.name,ss.session,ss.year
						  ...
						  and ss.session='$session'
						  and ss.courseid in(select courseid from tbl_marksentryfinal 						  where ss.deptid='$deptid'
											  and ss.semesterid='$semester'
											  and ss.session='$session'
	  ...
	  if(isset($_GET['show'])){
		  ...
		  $stq=$myDb->select("select*from tbl_stdinfo where deptname='$deptid' and semester='$semester' and session='$session' LIMIT $mainSTART,$mainPERPAGE");
	  }
	  while($stqf=$myDb->get_row($stq,'MYSQL_ASSOC')){	  
		  ...
		  echo "<table class='stdhead' cellpadding='0' cellspacing='0'>";
			...
			while($iscrsf=$myDb->get_row($iscrs,'MYSQL_ASSOC')){
			 $incs=$myDb->select("SELECT distinct c.id,c.coursecode,c.credit,m.stdid, m.examname, m.session, 
			  ...
			  //Determine Last status (pass,fail,reff) of students
				if($grade=="F"){
				 ...
				 $Lg=round($totalGP,2)/$cdrt;
					...
					if($fl<=3){
					  $stdsem=$myDb->select("SELECT*FROM tbl_stdinfo WHERE stdid='$incsf[stdid]'");
					  $stdsemf=$myDb->get_row($stdsem,'MYSQL_ASSOC');
						  $stdprm=$myDb->select("SELECT*FROM tbl_stdpromotiontrace 
												 WHERE stdid='$stdsemf[stdid]'
												 AND deptid='$stdsemf[deptname]'
												 AND semesterid='$stdsemf[stdcursemester]'
												 AND session='$stdsemf[session]' 
												 AND year='$year'");
						  $stdprmf=$myDb->get_row($stdprm,'MYSQL_ASSOC');							 
						  $stdcursem=$stdsemf['semester']+1;

						  if(empty($stdprmf['stdid'])){
							 if($myDb->insert_sql("INSERT INTO tbl_stdpromotiontrace(stdid,deptid,semesterid,session,year,opby,opdate,storedstatus)
												 VALUES('$stdsemf[stdid]','$stdsemf[deptname]','$stdcursem','$session','$year',
						   ...
						   $myDb->update_sql("UPDATE tbl_stdpromotiontrace SET 
						     						semesterid='$stdcursem',
													session='$session',
													year='$year'
