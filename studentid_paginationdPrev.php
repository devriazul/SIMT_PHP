...
<?php 
    ...
    if(!empty($searchid)){
		$sdq="SELECT s.id, s.stdid as StudentId, s.stdname AS Name, b.batchname AS Batch, d.name AS Department,h.name as HostelName, r.roomno as RoomNo, hsd.seatno as SNo FROM  `tbl_stdinfo` s inner join `tbl_batch` b on s.batch=b.id inner join `tbl_department` d on s.deptname=d.id inner join `tbl_hostelseatdetails` hsd on s.stdid=hsd.stdid inner join `tbl_hostel` h on hsd.hostelid=h.id inner join `tbl_hostelseat` r on hsd.roomid=r.id  WHERE s.storedstatus <>  'D' and s.stdid Like '$searchid%' order by id limit $start,$per_page ";
		$sdep=$myDb->dump_querySHD($sdq,'deallocatehostelseatdetails.php',$car['upd']);
    }else{		 
		$sdq="SELECT s.id, s.stdid as StudentId, s.stdname AS Name, b.batchname AS Batch, d.name AS Department,h.name as HostelName, r.roomno as RoomNo, hsd.seatno as SNo FROM  `tbl_stdinfo` s inner join `tbl_batch` b on s.batch=b.id inner join `tbl_department` d on s.deptname=d.id inner join `tbl_hostelseatdetails` hsd on s.stdid=hsd.stdid inner join `tbl_hostel` h on hsd.hostelid=h.id inner join `tbl_hostelseat` r on hsd.roomid=r.id  WHERE s.storedstatus <>  'D' order by id limit $start,$per_page ";
		$sdep=$myDb->dump_querySHD($sdq,'deallocatehostelseatdetails.php',$car['upd']);
    }
