...
include("config.php"); 
@$deptidF=mysql_real_escape_string($_POST['deptid'])?mysql_real_escape_string($_POST['deptid']):$_GET['deptid'];
@$sessionF=mysql_real_escape_string($_POST['session'])?mysql_real_escape_string($_POST['session']):$_GET['session'];

if(@$deptidF == 29 || @$deptidF == 30 || @$sessionF <= 1516){?>
 <!--new code start-->
 <?php 
	//echo "old logic"; 
	...
	<?php //All page heading (First,Last page not here)
			...
			echo "<div style='float:right; font-size:11px; width:auto; border:1px solid #333333; padding:3px;'>
				  <label>TC-Theory Continuous</label><br/>
				  <label>TF-Theory Final</label><br/>
				  <label>PC-Practical Continuous</label><br/>
				  <label>PF-Practical Final</label>
			
			</div>";
			echo "<div style='float:left; font-size:14px;width:700px;'>Technology Code and Name: {$dptqf['name']}<br/>Institute Code and Name: [50116] SAIC Institute of Management and Technology</div>"."<br/>";
	...
	<?php 	$crs=$myDb->select("SELECT distinct d.id,d.name,c.coursename,c.coursecode,s.name,ss.session,ss.year
		   ...
		   while($row=$myDb->get_row($crs,'MYSQL_ASSOC')){
				 ...
				 $ccode=substr($row['coursecode'],-5,$len);
		  ...
		  while($stqf=$myDb->get_row($stq,'MYSQL_ASSOC')){	  
			  ...
			  echo "<table class='stdhead' cellpadding='0' cellspacing='0'>";
				...
				if($sumpc>0){
					if($incsf['cont_assess_p'] == 0){$incsf['cont_assess_p'] = 1;}
	...
	<?php 	$crs=$myDb->select("SELECT distinct d.id,d.name,c.coursename,c.coursecode,s.name,ss.session,ss.year
		  ...
		  while($stqf=$myDb->get_row($stq,'MYSQL_ASSOC')){	  
			  ...
			  echo "<table class='stdhead' cellpadding='0' cellspacing='0'>";
				...
				$OTC=0;$OPC=0;$OTF=0;$OPF=0;
				if($sumtc>0){
					if($incsf['cont_assess_t'] == 0){$incsf['cont_assess_t'] = 1;}
					$OTC=(($sumtc/$incsf['cont_assess_t'])*100);
				}
				$sumpc=($iscrsf['jobexpr']+$iscrsf['hwmarks']+$iscrsf['jobexprreport']+$iscrsf['jobexprviva']+$iscrsf['attendancemarksprac']+$iscrsf['behaviormarks']);
				if($sumpc>0){
					if($incsf['cont_assess_p'] == 0){$incsf['cont_assess_p'] = 1;}
					$OPC=(($sumpc/$incsf['cont_assess_p'])*100);	   
				}
				$sumtf=$iscrsf['finalexammarks'];	
				if($sumtf>0){
					$OTF=(($sumtf/$incsf['f_exam_t'])*100);
				}
				$sumpf=($iscrsf['jobexprfinal']+$iscrsf['jobexprreportfinal']+$iscrsf['jobexprvivafinal']);  
				if($sumpf>0){
					$OPF=(($sumpf/$incsf['f_exam_p'])*100); 
				}
				
				$totalTFCB=($incsf['cont_assess_t']+$incsf['f_exam_t']);
				$totalPFCB=($incsf['cont_assess_p']+$incsf['f_exam_p']);
				
				$TCmarks = $incsf['cont_assess_t'];
				$TFmarks = $incsf['f_exam_t'];
				$PCmarks = $incsf['cont_assess_p'];
				$PFmarks = $incsf['f_exam_p'];
				$totalTFC=0;
				if($sumtf>0||$sumtc>0){
					$totalTFC=((($sumtf+$sumtc)/$totalTFCB)*100);
				}
