...
<table width="80%" border="0" cellspacing="0" cellpadding="0" class="tbl_repeat" style="padding:5px;margin:5px;" >
  ...
  <?php 
  $pr=$myDb->select("select*from tbl_product  where prtype like '$_GET[searchid]%' order by pname asc LIMIT $start, $per_page");
...
$query_pag_num = "SELECT count(*) AS count
	       FROM tbl_product where prtype like '$searchid%'
";
