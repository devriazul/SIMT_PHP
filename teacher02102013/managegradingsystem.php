...
//getting number of rows and calculating no of pages
  if(isset($_GET['searchid'])){ 
    $sql = "select * from  tbl_gradesystem WHERE name LIKE '%$_GET[searchid]%' and storedstatus<>'D'";  
	$rsd = mysql_query($sql);
    $count = mysql_num_rows($rsd);
    $pages = ceil($count/$per_page);

  }else{
    $sql = "select * from  tbl_gradesystem where storedstatus<>'D'";
	$rsd = mysql_query($sql);
    $count = mysql_num_rows($rsd);
    $pages = ceil($count/$per_page);
  }	
...
<script type="text/javascript">
   ...
   //Default Starting Page Results
	...
	$("#content1").load("gradingsystem_pagination.php?page=1", Hide_Load());
	...
	$("#pagination li").click(function(){
		...
		$("#content1").load("gradingsystem_pagination.php?page=" + pageNum, Hide_Load());
	});
