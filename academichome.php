      ...
      <tr>
        <td width="21%" valign="top" background="images/leftbg.jpg"><?php include("academicleft.php");?>         <br />
		  ...
		  /*
				...
				$st="SELECT ats.*, LEFT(TIMEDIFF(at.intime,ats.stdintime),1) as td, HOUR(TIMEDIFF(at.intime,ats.stdintime)) as hrs, MINUTE(TIMEDIFF(at.intime,ats.stdintime)) as mins FROM `tbl_attendancesettings` ats inner join tbl_attendance at on ats.accid=at.accid WHERE ats.accid=41 and at.efid='$userid' and at.edate=CURDATE()";
				$stq=$myDb->select($st);
				...
				$st="UPDATE tbl_attendance set astatus='$atndstatus' WHERE efid='$userid' and edate=CURDATE()";
