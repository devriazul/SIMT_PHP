...
$(document).ready(function() {	
	...
	$('a[name=modal]').click(function(e) {
		...
		var maskWidth = $(window).width();
			    $('#table_wrapper33').show().load('edit_settings.php?id='+tid);
...
$(document).ready(function(){
  ...
  $('#men').css({'font-family':'Verdana, Arial, Helvetica, sans-serif'}).css({'font-size':'12px'}).css({'font-weight':'normal'}).css({'color':'#999999'}).css({'border':'1px solid #999999'});
  $('#men').css({'background-color':'#D9F0FB'}).css({'padding':'10px'});
  $('#list-book').css({'font-family':'Verdana, Arial, Helvetica, sans-serif'}).css({'font-size':'12px'}).css({'font-weight':'normal'}).css({'color':'#999999'}).css({'border':'1px solid #999999'}).css({'padding':'10px'});
  ...
  $('#selfid').change(function(){  
    ...
    $('a[id=list-book]').click(function(e){
	 e.preventDefault();
	 $('#list-book').css({'padding':'10px'}).css({'background-color':'#D9F0FB'}).css({'color':'#999999'}).css({'border':'1px solid #999999'});
  ...
  */
  $('a[id=men]').click(function(e){
    e.preventDefault();
	  $('#men').css({'background-color':'#D9F0FB'}).css({'padding':'10px'}).css({'color':'#999999'}).css({'border':'1px solid #999999'});
  ...
  $('#button-big').click(function(){
	...
	$.post('ins_book.php?courseid='+courseid,arr,function(data){
	  $('#ins').css({'width':'500px','margin-left':'50px','padding':'10px',
	  				 'font-family':'verdana','font-size':'12px','background-color':'#999999'}).
	  		    css({'color':'#FFFFFF','height':'20px'}).hide().html(data).fadeIn();
	  //document.bfrm.reset();
	  $('#courseid').val('');
	  $('#isbnno').val('');
	  $('#author').val('');
...
<script>  
 $(document).ready(function() {
	$('#isbnno').focus('live',function(){
	  var deptid=$('#deptid').val();
	  var courseid=$('#courseid').val();
	  $.get("chkdeptbook.php?deptid="+deptid+"&courseid="+courseid,function(r){
		$('#chkbook').show("slow");
		$('#chkbook').addClass('msgshow');
		$('#chkbook').html(r);
...
<script language="javascript">
  $(document).ready(function(){
	 $('#prtn').click(function(){
	    var crsname=$('#crsname').val();
		var listb=$('#listb').val();
		var thePopup = window.open("BookRpt.php?crsname="+crsname+"&listb="+listb,"Book Issue & Return Information","location=0,titlebar=0,menubar=0,addressbar=no,height=700,width=900","fullscreen=yes" );
			thePopup.print();
...
<div id="boxes">
       ...
       <a href="#" class="close"  style="padding-left:530px;"><img src="images/closebox.png" width="20" height="20" border="0" /></a>

       <div id="table_wrapper22" style="margin-bottom:110px;"></div>
       <div id="table_wrapper33" style="margin-bottom:110px;"></div>
       <div id="table_wrapper" style="margin-bottom:110px;"></div>
     </div>
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" id="tblleft">
      ...
      <tr>
        <td width="21%" valign="top" background="images/leftbg.jpg"><?php include("left.php");?><br />
		  ...
		  <div style="float:left;margin-left:30px;" ><a href="#" id="men">Book Entry Form</a> <a href="#" id="list-book">List View</a><div style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:10px; padding-left:10px; width:300px; float:right; ">[Esc->List View || Shift+F1->Book Entery Form][*=Mandotory Field]</div></div>
		  <div style=" position:relative;border:5px solid #999999;top:28px;"></div>
...
<div id="top-search-div"> 
		   <form method="post" id="srs">
             ...
             <label><input type="text" id="crsname" name="crsname" placeholder="Search By" /></label>
			 <label><input type="button" name="subs" id="submit-btn" value="Search" /></label>
			 <label> <input type="button" name="prtn" id="prtn" value="Print" class="button-class" /></label>		   
           </form>
		</div>
...
<div style="padding-left:50px;width:500px;" id="book-form">	

<div id="chkbook" style="display:none;">
  <div id="imgcls" align="right"><img id="closemsg" src="images/closebox.png"/></div>
...
<table width="700" border="0" cellspacing="0" cellpadding="0" id="book">
 ...
 <tr>
  ...
  <tr>
    <td colspan="4"><div style="margin-left:150px;"><input type="button" value="Add Book" id="button-big" class="button-class"></div></td>
   </tr>
</table>
