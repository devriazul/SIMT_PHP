...
<?php 	
	...
	$tbbq=$myDb->select("select ifnull(count(*),0) totalbook,courseid from tbl_bookissue WHERE stdid='$stdid' and irstatus='ISSUE' and `return`<>'R'");
	$tbbf=$myDb->get_row($tbbq,'MYSQL_ASSOC');
	
	$crrs1=$myDb->select("SELECT*FROM tbl_courses WHERE id='$tbbf[courseid]'");
	$crrsf1=$myDb->get_row($crrs1,'MYSQL_ASSOC');
